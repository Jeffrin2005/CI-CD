...
function ConsumerSidebar({ user, activeTab, setActiveTab }) {
    ...
    return (
        <aside className="w-64 bg-gray-900 border-r border-white/10 flex flex-col fixed h-full z-10">
            ...
            <div className="flex-1 py-6 px-4 space-y-2">
                <p className="text-gray-500 text-xs uppercase tracking-widest mb-4 px-2">Menu</p>

                <button
                    onClick={() => setActiveTab("browse")}
                    className={`w-full text-left px-4 py-3 font-bold rounded-lg transition-colors ${activeTab === "browse" ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "text-gray-400 hover:text-white hover:bg-white/5 border border-transparent"}`}
                >
                    🍽 Browse Hotels
                </button>
            ...
            <div className="p-4 border-t border-white/10">
                <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-full bg-amber-500 flex items-center justify-center text-black font-extrabold text-lg">
